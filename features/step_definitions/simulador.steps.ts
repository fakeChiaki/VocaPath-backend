...
Given('que el estudiante tiene registrados los siguientes puntajes bajos por materia:', function (dataTable) {
    // Escenario 5: Reutilizamos tu misma lógica para mapear los puntajes bajos
    const rows = dataTable.hashes();
    userScores = rows.map((row: any) => ({
        factor: LABEL_TO_FACTOR[row.Materia],
        value: parseFloat(row.Puntaje),
    ...
    // Usando tus nombres en inglés para mantener la consistencia de tu código
    careerData.weights = { nem: 10, ranking: 20, language: 10, math: 20, science: 40 };
});
